...
public class SellCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        // /sellgui debug - Debug item in hand
        if (args.length == 1 && args[0].equalsIgnoreCase("debug")) {
            ...
            if (itemInHand == null || itemInHand.getType() == Material.AIR) {
                player.sendMessage(color("&cYou must be holding an item to debug!"));
                return true;
            }

            player.sendMessage(color("&eDebugging item in hand... Check console for details."));
            ...
            player.sendMessage(color("&aItem Type: &f" + type.name()));
            player.sendMessage(color("&aIdentifier: &f" + (identifier != null ? identifier : "null")));
            player.sendMessage(color("&aDisplay Name: &f" + displayName));
